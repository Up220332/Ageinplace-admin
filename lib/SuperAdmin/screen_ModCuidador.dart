...
/// *****************************************************************************
/// Funcion que modifica los datos de un cuidador
///****************************************************************************
class ModCuidadorScreen extends StatefulWidget {
  final String role;
  final Cuidadores cuidador;

  const ModCuidadorScreen({
    super.key,
  ...
  @override
  State<ModCuidadorScreen> createState() => _ModCuidadorScreenState();
}

class _ModCuidadorScreenState extends State<ModCuidadorScreen> {
  ...
  TipodeCuidador? _selectedTipoCuidador; // Para el valor seleccionado
  ...
  final Map<String, Map<String, String>> translations = {
    'es': {
      ...
      'header': 'Modifique los datos del cuidador',
    ...
    'en': {
      ...
      'header': 'Modify the caregiver data',
...
Map<String, String> caregiverTypeTranslations = {
  ...
  'Otros': 'Otros',
  // Añade también los que aparecen en tu imagen:
  'Cónyuge (Cuidador Informal)': 'Cónyuge (Cuidador Informal)',
  'Hermano/a (Cuidador Informal)': 'Hermano/a (Cuidador Informal)',
  'Hijo/a (Cuidador Informal)': 'Hijo/a (Cuidador Informal)',
  'Madre (Cuidador Informal)': 'Madre (Cuidador Informal)',
  'Padre (Cuidador Informal)': 'Padre (Cuidador Informal)',
};
...
String getTranslatedCaregiverType(String type) {
  ...
  if (currentLocale == 'en') {
    ...
    return enTranslations[type] ?? type;
  } else {
    // En español, devolver el original
    return type;
  }
}
  ...
  Future<String> getTipoCuidador() async {
    ...
    setState(() {
      ...
      _selectedTipoCuidador = TipoCuidadorList.firstWhere(
        (tipo) => tipo.TipoCuidadorTabla == widget.cuidador.TipoCuidador,
        orElse: () => TipoCuidadorList.isNotEmpty
            ? TipoCuidadorList.first
            : TipodeCuidador(1, 'Cuidador formal'),
      );
    });
    ...
    return WillPopScope(
      onWillPop: () async {
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) => CuidadoresScreen(role: widget.role),
          ),
        );
        return false;
      },
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          Icon(Icons.person, color: colorPrimario, size: 24),
                    ...
                    // En el dropdown, cambia la parte de los items:
...
Container(
  ...
  child: DropdownButtonFormField<TipodeCuidador>(
    ...
    items: TipoCuidadorList.map((tipo) {
      return DropdownMenuItem(
        value: tipo,
        child: Text(
          getTranslatedCaregiverType(tipo.TipoCuidadorTabla), // <-- APLICAR TRADUCCIÓN AQUÍ
        ),
      );
    }).toList(),
    onChanged: (TipodeCuidador? newValue) {
      setState(() {
        _selectedTipoCuidador = newValue;
        if (newValue != null) {
          CodTipoCuidador = newValue.CodTipoCuidador;
          if (newValue.TipoCuidadorTabla == 'Otros') {
  ...
  Future<void> ContinuaButton() async {
    var newcuidadorviviendaOk = await DBPostgres().DBModCuidadorVivienda(
      widget.cuidador.CodUsuarioCuidador,
    ...
    if (newcuidadorviviendaOk == true) {
      _showSuccessDialog();
    } else if (newcuidadorviviendaOk.toString().contains(
      'Ya existe la llave',
    )) {
